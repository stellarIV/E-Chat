...
export const get = query({args: {
    ...
    handler: async (ctx, args)=>{
        ...
        const allConversationMemberships = await ctx.db.query
        ("conversationMembers").withIndex
        ("by_conversationId",q=>q.eq
        ("conversationId",args.id)).collect()
...
//                 allConversationMemberships.filter(membership => membership.memberId !== currentUser._id))).map(async membership => {
